...
function calcResult(answers) {
  ...
  return winners.length > 2 ? [winners[0], winners[1]] : winners
}
...
export default function Quiz() {
  ...
  if (phase === 'result') {
    const winners = calcResult(answers)

    if (winners.length === 1) {
      const resultData = RESULTS[winners[0]]
      const collection = collections.find((c) => c.id === winners[0])
    ...
    const colA = collections.find((c) => c.id === winners[0])
    const colB = collections.find((c) => c.id === winners[1])
    return <DualResultPhase collectionA={colA} collectionB={colB} onReset={handleReset} />
  }
}
...
function DualResultPhase({ collectionA, collectionB, onReset }) {
  const navigate = useNavigate()
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: '720px', width: '100%', textAlign: 'center' }}>
        <div style={{ animation: 'fadeInUp 0.6s ease 0.1s both', marginBottom: '2.5rem' }}>
          <span style={{
            display: 'block',
            fontFamily: "'Raleway', sans-serif",
            fontSize: '0.7rem',
          ...
          <p style={{
            ...
            Sua energia está dividida entre duas essências. Deixe que o momento escolha por você.
          </p>
        ...
        <div style={{
          ...
          {[collectionA, collectionB].map((col) => (
            <div key={col.id} style={{
              border: `1px solid rgba(${parseInt(col.color.slice(1,3),16)}, ${parseInt(col.color.slice(3,5),16)}, ${parseInt(col.color.slice(5,7),16)}, 0.35)`,
              borderRadius: '2px',
              ...
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.background = col.color; e.currentTarget.style.color = '#1E1035' }}
                onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = col.color }}
              >
                Conhecer {col.name}
              </button>
            </div>
